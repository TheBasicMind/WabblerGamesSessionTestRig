...
public class Storage {
    ...
    static fileprivate func getURL(for directory: Directory, fileName: String?) throws -> URL {
    ...
    /// Remove all files at specified directory
    static func retreiveAllOfType<T: Decodable>(_ type: T.Type, directory: Directory) throws -> [T] {
        let url = try getURL(for: directory, fileName: nil)
        let contents = try FileManager.default.contentsOfDirectory(at: url, includingPropertiesForKeys: nil, options: [])
        var objects = [T]()
        for fileUrl in contents {
